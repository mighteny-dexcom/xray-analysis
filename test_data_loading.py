...
def test_data_loading():
    ...
    try:
        ...
        if lot1_manifest.exists():
            ...
            print(f"✓ Lot 1 manifest loaded successfully")
            print(f"  Records count: {len(df1)}")
            ...
            print("✗ Lot 1 manifest not found")
    except Exception as e:
        print(f"✗ Error loading Lot 1: {e}")
    ...
    try:
        if lot2_manifest.exists():
            ...
            print(f"✓ Lot 2 manifest loaded successfully")
            print(f"  Records count: {len(df2)}")
            ...
            print("✗ Lot 2 manifest not found")
    except Exception as e:
        print(f"✗ Error loading Lot 2: {e}")
    ...
    try:
        if lot3_manifest.exists():
            ...
            print(f"✓ Lot 3 manifest loaded successfully")
            print(f"  Records count: {len(df3)}")
            ...
            print("✗ Lot 3 manifest not found")
    except Exception as e:
        print(f"✗ Error loading Lot 3: {e}")
    ...
    for lot_num, images_dir in [("1", lot1_images_dir), ("2", lot2_images_dir), ("3", lot3_images_dir)]:
        try:
            if images_dir.exists():
                ...
                print(f"✓ Lot {lot_num} images folder accessible")
                ...
                if image_files:
                    ...
                    try:
                        ...
                        print(f"  ✓ Sample image opened successfully: {sample_image.name}")
                        print(f"    Size: {img.size}, Mode: {img.mode}")
                    except Exception as e:
                        print(f"  ✗ Error opening sample image: {e}")
            else:
                print(f"✗ Lot {lot_num} images folder not found")
        except Exception as e:
            print(f"✗ Error accessing Lot {lot_num} images: {e}")
    ...
    print(f"Total records across all lots: {total_records}")
    print()
    ...
    if all([lot1_manifest.exists(), lot2_manifest.exists(), lot3_manifest.exists()]):
        ...
        print(f"  BT/pBT labels: {bt_count}")
        print(f"  Functional/blank labels: {functional_count}")
