"""
X-Ray Analysis Model Training - Fixed Version v3
Handles ConcatDataset properly with correct batching
"""
...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Create grayscale (single channel) dummy images for X-rays
        img_tensor = torch.zeros(1, 299, 299)
        
        # Return as tuple (image, label) - standard PyTorch format
        return {
            'image': img_tensor,
            'label': self.labels[idx]  # Return float directly, not tensor
        }
...
def train_model():
    ...
    print("=" * 60)
    print("X-RAY ANALYSIS MODEL TRAINING (FIXED VERSION v3)")
    print("=" * 60)
    print()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # NO .unsqueeze(1) needed - already single channel grayscale
