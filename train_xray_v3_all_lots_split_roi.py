# train_xray_v3_all_lots_split_roi.py
# Train ResNet18 transfer-learning model using Lot1, Lot2, Lot3
# Validation is stratified within each lot.
# Includes optional ROI crop for battery-tab region.
...
# ROI crop for battery-tab region
USE_ROI_CROP = True
ROI_BOX = (450, 350, 650, 500)  # (left, top, right, bottom)

# Screening model-selection goal:
# Prioritize minimizing false negatives while keeping false positives reasonable.
MIN_ACCEPTABLE_SPECIFICITY = 0.80

# Candidate operating threshold.
# You can later threshold-sweep this model separately.
...
def apply_roi_crop(img):
    """
    Applies ROI crop if enabled.
    ROI_BOX format: (left, top, right, bottom)
    """
    ...
    if left < 0 or top < 0 or right > width or bottom > height:
        raise ValueError(
            f"ROI_BOX {ROI_BOX} is outside image bounds. "
            f"Image size is width={width}, height={height}."
        )

    if right <= left or bottom <= top:
        raise ValueError(
            f"Invalid ROI_BOX {ROI_BOX}. "
...
def stratified_split_indices(labels, val_fraction=0.20, seed=42):
    """
    Manual stratified split without sklearn.
    Keeps positives and negatives represented in both train and validation.
    """

    rng = random.Random(seed)

    pos_indices = [i for i, label in enumerate(labels) if int(label) == 1]
    neg_indices = [i for i, label in enumerate(labels) if int(label) == 0]

    rng.shuffle(pos_indices)
    rng.shuffle(neg_indices)

    n_pos_val = max(1, int(round(len(pos_indices) * val_fraction))) if len(pos_indices) > 0 else 0
    n_neg_val = max(1, int(round(len(neg_indices) * val_fraction))) if len(neg_indices) > 0 else 0
...
class TransformSubset(Dataset):
    """
    Allows train and validation subsets to use different transforms
    while sharing the same underlying filenames/labels.
    """
    ...
    def __init__(self, base_dataset, indices, transform):
        ...
        self.filenames = [base_dataset.filenames[i] for i in indices]
        self.lot_name = base_dataset.lot_name
    ...
    def __getitem__(self, local_idx):
        ...
        filename = self.base_dataset.filenames[base_idx]
        label = self.base_dataset.labels[base_idx]
        lot = self.base_dataset.lot_name
        img_path = self.base_dataset.image_dir / filename
...
def train_all_lots_model():
    ...
    print("=" * 60)
    print("TRAIN RESNET18 TRANSFER MODEL - ALL LOTS STRATIFIED SPLIT")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("CREATING STRATIFIED WITHIN-LOT SPLITS")
    print("=" * 60)

    for lot_name in excel_files.keys():
        base_dataset = SimpleMultiLotDataset(
        ...
        train_pos = sum(int(x) for x in train_subset.labels)
        train_neg = len(train_subset) - train_pos

        val_pos = sum(int(x) for x in val_subset.labels)
        val_neg = len(val_subset) - val_pos

        print(f"\n[SPLIT] {lot_name}")
        print(f"        Train: {len(train_subset)} total | Pos={train_pos}, Neg={train_neg}")
        print(f"        Val:   {len(val_subset)} total | Pos={val_pos}, Neg={val_neg}")
    ...
    print(f"Val positive BT/pBT:          {val_positive}")
    print(f"Val negative functional:      {val_negative}")
    print(f"Decision threshold:           {DECISION_THRESHOLD}")
    print(f"Min acceptable specificity:   {MIN_ACCEPTABLE_SPECIFICITY}")
    print(f"Validation fraction:          {VAL_FRACTION}")
    print(f"Use ROI crop:                 {USE_ROI_CROP}")
    ...
    roi_suffix = "roi" if USE_ROI_CROP else "full"

    best_model_path = models_dir / f"best_screening_resnet18_all_lots_split_{roi_suffix}.pth"
    fallback_model_path = models_dir / f"best_fallback_resnet18_all_lots_split_{roi_suffix}.pth"
    last_model_path = models_dir / f"last_resnet18_all_lots_split_{roi_suffix}.pth"
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Fallback model: best balanced accuracy, regardless of specificity constraint
        ...
        if is_better_fallback_model:
            ...
            torch.save({
                ...
                "split_type": "all_lots_stratified_within_lot",
        ...
        # Primary screening model: specificity-constrained, prioritize sensitivity
